...
def feature_extraction_pipeline(image, features_file_path,
                                mother_wavelet, distances, angles, label,
                                orientations, pixels_per_cell,
                                cells_per_block):
    data_point10 = PipelineDataPoint(image, 10)
    ...
    strategy3 = PipelineStrategy(wavelet_features_extraction, [10, 12], 3)
    strategy4 = PipelineStrategy(color_features_extraction, [10], 4)
    strategy5 = PipelineStrategy(texture_features_extraction, [10, 13, 14], 5)
    strategy6 = PipelineStrategy(hog_features_extraction, [10, 16, 17, 18], 6)
...
def dataset_feature_extraction(path_to_dataset, feature_dir_path,
    ...
    def perform_extraction_on_files(input):
        for load_path, save_path in zip(input[0], input[1]):
            ...
            list_of_patches = divide_into_patches(image, 2, 2)
            filename, file_extension = os.path.splitext(save_path)
            mkdir_p(filename)
            for index, patch in enumerate(list_of_patches):
                save_path = os.path.join(filename, "patch" + str(index) + ".json")
                feature_extraction_pipeline(patch, save_path, input[2],
